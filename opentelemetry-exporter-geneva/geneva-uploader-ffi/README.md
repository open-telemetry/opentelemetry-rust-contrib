...
C/C++ FFI layer over `geneva-uploader`. Exposes a stable C ABI so that applications written in other languages can encode and upload telemetry to Geneva without pulling in the Rust OTel SDK.

## Ingestion paths

### 1. OTLP bytes path

Pass a serialized `ExportLogsServiceRequest` (or `ExportTraceServiceRequest`) protobuf. Rust deserializes it and encodes to Geneva Bond+LZ4.
...
Use this when you are already working with OTLP protobuf bytes or OTLP-generated code and want to pass those serialized payloads through directly.

### 2. Direct log record path (zero-copy)

Pass a flat C array of `GenevaLogRecordC` structs. Rust reads the fields directly from C memory — no intermediate OTLP serialization, no copies of string data.
...
Use this when you have a custom in-memory representation for your log records and want to avoid converting them to OTLP protobuf bytes first.
